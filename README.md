...
**PyPy - это тот интерпретатор питона, которого мы заслуживаем. Но не тот, который нужен нам всегда.**
...
[Резюме](https://github.com/Asklepiad/PyPy_vs_CPython/tree/main#резюме)

[Источники информации и полезные ссылки](https://github.com/Asklepiad/PyPy_vs_CPython/tree/main#источники-информации-и-полезные-ссылки)
...
PyPy эффективен в ресурсоемких задачах, выполненных на чистом питоне. Он не годится для быстроисполняемых скриптов, и скриптов в которых важную роль в оптимизации имеют Си-ассоциированные библиотеки. У PyPy есть ряд нюансов, связанных с удалением неиспользующихся файлов, использованием метода `getitem` (рассматривается в ноутбуках) и другого. Подробнее про разоичия между CPython и PyPy можно почитать [в документации](https://doc.pypy.org/en/latest/index.html).

Одним словом, PyPy это что-то быстрое, красивое и интересное. Но подходящее далеко не для всех повседневных задач. Что-то наподобие такого:

![Бэтоцикл](./pictures/batcycle.jpg)

Наверное, вы не поедете за хлебом в булочном на таком. Но для специальных задач, вроде погони за Джокером - это самое то.
